    ...
    if request.method == "POST":
        ...
        if password == confirm_password:
            try:
                ...
                data = {"displayName": username, "verified": "0",
                        'email': email, 'password': password}
                db.child("users_profle").child(uid).child("details").set(data)
                print("Registered")
                user_login = authe.sign_in_with_email_and_password(
                    email, password)
    ...
    if request.method == "POST":
        ...
        if(ev_submit == "Apply"):
            ...
            db.child('Created Events').child(year).child(month).child(date).child(
                event_name).child('Registered').child(uid).update(data)
            return HttpResponseRedirect('/users')
        if(ev_submit == "info"):
            event_venue = request.POST.get('event_venue')
            contact_number = request.POST.get('contact_number')
...
def dashboard_edit(request):
    if request.method == "POST":
        ...
        db.child("users_profle").child(uid).child("details").update(data)
        return HttpResponseRedirect('/users')
    else:
